...
import { CheckCircle2, XCircle, Clock, User, MoreHorizontal } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const UserApprovalTable: React.FC<UserApprovalTableProps> = ({ users, onApprove, onReject }) => {
  if (users.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 bg-white rounded-[40px] border-2 border-dashed border-neutral-100">
        ...
        <div className="space-y-2">
          <h3 className="text-2xl font-black tracking-tight text-neutral-900">No users found</h3>
          <p className="text-neutral-500 font-medium max-w-xs mx-auto">
            All users in this category have been processed or none match your search.
          </p>
        </div>
      </div>
    );
        ...
        <Table>
          <TableHeader className="bg-neutral-50/50">
            <TableRow className="hover:bg-transparent border-neutral-100">
              <TableHead className="w-75 h-16 px-8 text-[10px] font-black uppercase tracking-widest text-neutral-400">User</TableHead>
              <TableHead className="h-16 px-8 text-[10px] font-black uppercase tracking-widest text-neutral-400">Roles</TableHead>
              <TableHead className="h-16 px-8 text-[10px] font-black uppercase tracking-widest text-neutral-400">Registered</TableHead>
              <TableHead className="h-16 px-8 text-[10px] font-black uppercase tracking-widest text-neutral-400">Status</TableHead>
              <TableHead className="h-16 px-8 text-right text-[10px] font-black uppercase tracking-widest text-neutral-400">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {users.map((user) => (
              <TableRow key={user.id} className="hover:bg-neutral-50/50 transition-colors border-neutral-100">
                <TableCell className="p-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-0.5">
                      <p className="font-black text-neutral-900 tracking-tight">{user.fullName}</p>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button size="sm" className="rounded-xl bg-emerald-600 hover:bg-emerald-700 font-bold gap-2">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button size="sm" variant="ghost" className="rounded-xl font-bold text-rose-600 hover:bg-rose-50 gap-2">
