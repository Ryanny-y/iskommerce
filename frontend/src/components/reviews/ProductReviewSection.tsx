...
export const ProductReviewSection: React.FC<ProductReviewSectionProps> = ({
  ...
  return (
    <div className="space-y-6 p-6 rounded-[32px] bg-neutral-50/50 border border-neutral-100" id={id}>
      <div className="flex items-center gap-4" id={`${id}-product-info`}>
        <div className="w-20 h-20 rounded-2xl overflow-hidden bg-neutral-100 border border-neutral-200" id={`${id}-image-container`}>
          {productImage ? (
            <img 
              src={productImage} 
              alt={productName} 
        ...
        <div className="space-y-1" id={`${id}-details`}>
          ...
          <p className="text-emerald-600 font-black" id={`${id}-price`}>
            ₱{productPrice}
          </p>
        </div>
      </div>
